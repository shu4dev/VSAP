...
text \<open> Shallowly embedded arithmetic expressions \<close>

 \<comment> \<open>In a shallow embedding, two expressions are equal if they evaluate to the same thing.\<close>
lemma "x + x = 2 * (x::nat)" by simp

text \<open> Deeply embedded arithmetic expressions \<close>

datatype arith_exp =
    Var string
    | Const nat
    | Add arith_exp arith_exp
    | Mul arith_exp arith_exp

fun arith_reduce :: "(string \<Rightarrow> nat) \<Rightarrow> arith_exp \<Rightarrow> nat" where
  "arith_reduce st (Var s) = st s"
| "arith_reduce st (Const n) = n"
| "arith_reduce st (Add n m) = arith_reduce st n + arith_reduce st m"
| "arith_reduce st (Mul n m) = arith_reduce st n * arith_reduce st m"

\<comment> \<open>Deep embeddings can have syntactically different expressions... \<close>
lemma "Add (Var ''x'') (Var ''x'') \<noteq> Mul (Const 2) (Var ''x'')"
  by simp

\<comment> \<open>...whose semantics are nonetheless equal: \<close>
lemma "arith_reduce st (Add (Var ''x'') (Var ''x'')) =
    arith_reduce st (Mul (Const 2) (Var ''x''))"
  by simp
